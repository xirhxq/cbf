...
CONFIG_DIR = pathlib.Path("config/generated_iter6_goal_diversion")
DATA_DIR = pathlib.Path("data/iter6_goal_diversion")
OUT_DIR = pathlib.Path("data/iter6_goal_diversion")
TRIALS = [(7, 20261224), (12, 20261229)]
BASE_SEED = 20261217
PREDICTIVE_RESERVE_MARGIN = 25.0


def build_spec_config(base, trial, seed, label, divert=False, sd=False):
...
def main() -> int:
    ...
    for trial, seed in TRIALS:
        ...
        for label, divert, sd in [
            ("AS_HOCBF_TASK_RESERVE", False, False),
            ("AS_HOCBF_TASK_RESERVE_SD", False, True),
            ("AS_HOCBF_TASK_RESERVE_DIVERT", True, False),
        ]:
            config = build_spec_config(base, trial, seed, label, divert=divert, sd=sd)
    ...
    out_csv = OUT_DIR / "iter6_seed_results.csv"
