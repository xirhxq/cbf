...
namespace {
void runPathFixture(gf::Task28LayerPath::Kind kind) {
    for (int variant:{0,1}) {
        ...
        gf::Task28LayerPath path(goal,a,b,kind);
        ...
        std::cout<<"TASK28_FOCUSED "<<nlohmann::json({{"path_kind",static_cast<int>(kind)},{"variant",variant},{"ticks",1200},{"maximum_tracking_error_m",maxerror},{"maximum_speed_mps",maxspeed},{"states",record}}).dump()<<'\n';
        // This fixture checks actual gates and task identity; convergence is
        // measured, not assumed from the nominal all-pair geometry.
    }
}
}
TEST_CASE("Task28 full fourteen-member path fixture with offset and nonzero velocity") {runPathFixture(gf::Task28LayerPath::Kind::Serial);}
TEST_CASE("Task28 common-finish full fourteen-member fixture") {runPathFixture(gf::Task28LayerPath::Kind::CommonFinish);}
TEST_CASE("Task28 centered-frame full fourteen-member fixture") {runPathFixture(gf::Task28LayerPath::Kind::CenteredFrame);}
