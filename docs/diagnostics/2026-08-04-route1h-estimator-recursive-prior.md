...
2. **1 行（f153 r14）：局部 reduced-cost 门拒绝（修正）**。实测该行
   有 8 个起点、全部收敛到同一解：估计 (-460.8, 1459.1)，距真值
   仅 **1.93 m**（准确），但 `recompute_local_candidate_diagnostics`
   的 `reduced_cost = cost/(active-2) = 11.19/1 = 11.19 > 9.0`
   （`reacquisition_reduced_cost_exceeds_nine`）→ 候选全被丢。
   残差分解：ref11/12/13 的 range residual 为 +2.84/-3.05/-4.99 m，
   LOS 方差 2.11/4.31/4.10 m²，whitened 项 3.42+2.04+5.73=11.19；
   通过 9.0 门只需方差再放大 **1.24×**。本质同样是**锚点协方差
   欠估使 whitened cost 略超冻结门**，不是测距噪声（σ=0.5 m 的
   贡献可忽略）。

两类的共同根因：任务边缘 UAV 锚点的累积估计误差，使量测集与
冻结的 innovation/reduced-cost 阈值不完全一致；估计器 fail-closed。
