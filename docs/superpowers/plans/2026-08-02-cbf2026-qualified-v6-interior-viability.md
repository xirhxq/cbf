...
- Every implementation task uses a vertical TDD cycle: focused RED test with the intended assertion failure, minimal GREEN implementation, focused regression, full relevant regression, and `git diff --check`; after explicit approval, freeze a scoped implementation commit, obtain independent review, and use new scoped fix commits/re-reviews until C0/I0/M0. Task 6b additionally requires its committed pre-implementation amendment review before any RED or source edit, and its separate post-implementation review after the exact 18-file candidate commit.
