...
The prospectively paired cells are:

```text
dynamic DAG x strict previous
dynamic DAG x deployment restart before first acquisition
fixed references x strict previous
fixed references x deployment restart before first acquisition
```

All four strict-versus-restart cells are generated by one committed
executable in one registered run.
The immutable bundle is an external reproducibility anchor,
not the causal comparator.
Before the policy comparison is admissible,
the new strict-previous arm must reproduce every immutable baseline process
key and every scientific row value after removing only the prospectively
added policy and initialization-provenance fields.
Any strict-arm mismatch is an implementation-integrity failure and stops the
comparison.
...
| Category | Predicate |
| --- | --- |
| `exclusive_prior_self_estimate_missing_or_malformed` | The preceding row for the same seed, graph case, and UAV has no finite two-dimensional estimate and every recorded noisy range is finite |
| `prior_missing_and_recorded_measurement_invalid` | The preceding estimate is missing or malformed and at least one recorded noisy range is missing or non-finite |
| `recorded_measurement_missing_or_nonfinite` | The preceding estimate is finite and at least one recorded noisy range is missing or non-finite |
| `unresolved_compound_validator` | None of the above predicates is supported by the recorded fields |
...
`invalid_input_or_numeric` totals from the completed failure audit by
separating the exact malformed-WNLS-input reason from every other raw reason.
...
1. every baseline row with the exact malformed-WNLS-input reason is
   classified once;
2. the exact-reason count plus all other non-upstream invalid reasons
   reconciles to `invalid_input_or_numeric`;
3. at least 95% of the exact-reason rows are
   `exclusive_prior_self_estimate_missing_or_malformed`;
4. zero exact-reason rows are
   `prior_missing_and_recorded_measurement_invalid`;
5. no source-integrity,
   ordering,
   reconciliation,
   non-finite JSON,
   or disk-limit error occurs; and
6. an independent review finds no Critical or Important issue.
...
no finite preceding estimate and
  no estimate has ever been acquired -> configured deployment position
no finite preceding estimate after
  an estimate has been acquired      -> preserve strict-previous behavior
```
...
The replay tracks `ever_acquired_finite` independently for each
`(seed, initialization_policy, graph_case, robot_id)`.
It becomes true only after `_valid_prior_result` accepts a retained result and
never returns to false.
The restart changes only the initial estimate supplied after reference-input
validation succeeds and only while that flag is false.
...
- `ever_acquired_finite_before_attempt` and the initialization source for
  every process row;
...
The paired strict-versus-restart replay is a new evidence bundle.
...
The primary outcome is the paired seed-level difference in that exact event
fraction:
...
-
p^{\mathrm{strict}}_{\mathrm{direct},s}.
\]

The paired analyzer schema is frozen as
...
2. the aggregate dynamic exact-event count falls by at least 90%.

Both criteria are frozen before the intervention run.
The broader `invalid_input_or_numeric` class is reported separately as a
prespecified component check,
but it cannot replace the exact primary event.
...
2. classifier reconciliation,
   mutually exclusive compound classification,
   unresolved,
   ordering,
   source-mutation,
   and non-finite-input failures;
3. baseline policy preserving the existing null warm-start behavior;
4. restart policy selecting deployment only before the first finite estimate;
5. both policies selecting the previous estimate when it is finite;
6. restart policy preserving strict behavior for an artificial missing prior
   after `ever_acquired_finite` becomes true;
7. upstream-unavailable reference input still short-circuiting before WNLS;
8. identical active and fixed reference membership across policies;
9. identical measurement noise for every paired key;
10. normalized strict-policy rows exactly reproducing the immutable baseline;
11. exact preservation of WNLS,
   FIM,
   epsilon,
   and graph-selection constants;
12. paired-analysis exact-event denominator,
    seed,
    bootstrap,
    hierarchy,
...
14. a compact end-to-end fixture covering all four prospective cells.
