...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: GrandFinaleTask10p11tOneStep "
    ...
    try {
        ...
        const nlohmann::json snapshot =
            gf::readTask10p11sSnapshot(input_path);
        nlohmann::json output = gf::runTask10p11tOneStepSuccessorAudit(
        ...
        return output.value("successor_feasible", false) ? 0 : 4;
    } catch (const std::exception& error) {
        std::cerr << "Task 10.11t one-step successor audit failed: "
                  << error.what() << '\n';
        return 3;
    }
}
