...
GridWorld::GridWorld(pd _x_lim, int _x_num,
...
int GridWorld::get_num_in_xlim(double _x, std::string _mode) {
    return get_num_in_lim(_x, x_lim, x_num, _mode);
}

int GridWorld::get_num_in_ylim(double _y, std::string _mode) {
    return get_num_in_lim(_y, y_lim, y_num, _mode);
}

int GridWorld::get_ind(int _x_ind, int _y_ind) const {
    int ind = _x_ind * y_num + _y_ind;
    return ind;
}

int GridWorld::get_x_ind(int _ind) const {
    int x_ind = _ind / y_num;
    return x_ind;
}

int GridWorld::get_y_ind(int _ind) const {
    int y_ind = _ind % y_num;
    return y_ind;
}

bool GridWorld::get_result(Point _p) {
...
bool GridWorld::get_result(int _ind) {
    return vis[_ind] == true;
}

bool GridWorld::get_result(int _x_ind, int _y_ind) {
...
void GridWorld::set_result(Point _p, bool _res) {
    int x_ind = get_num_in_xlim(_p.x);
    int y_ind = get_num_in_ylim(_p.y);
    vis[get_ind(x_ind, y_ind)] = _res;
}

void GridWorld::set_result(int _x_ind, int _y_ind, bool _res) {
    vis[get_ind(_x_ind, _y_ind)] = _res;
}

void GridWorld::set_result(int _ind, bool _res) {
    vis[_ind] = _res;
}

double GridWorld::get_pos_in_lim(int _ind, pd _lim, int _sz) {
    double ratio = 1.0 * _ind / _sz;
    double pos = _lim.first * (1.0 - ratio) + _lim.second * ratio;
    return pos;
}

double GridWorld::get_xpos_in_xlim(int _x_ind) {
    return get_pos_in_lim(_x_ind, x_lim, x_num);
}

double GridWorld::get_ypos_in_ylim(int _y_ind) {
    return get_pos_in_lim(_y_ind, y_lim, y_num);
}

Point GridWorld::get_point_in_area(int _x_ind, int _y_ind) {
    return {get_xpos_in_xlim(_x_ind), get_ypos_in_ylim(_y_ind)};
}

Point GridWorld::get_point_in_area(int _ind) {
    return {get_xpos_in_xlim(get_x_ind(_ind)), get_ypos_in_ylim(get_y_ind(_ind))};
}

double GridWorld::get_res_in_polygon(Polygon _p) {
    double res = 0;
...
json GridWorld::set_res_in_polygon(Polygon _p, bool _res, bool _update_json) {
    json ret = json::array();
    pd x_lim_pd = _p.get_x_limit(1.0), y_lim_pd;
    pd x_ind_pd, y_ind_pd;
    x_ind_pd.first = get_num_in_xlim(x_lim_pd.first, "ceil");
...
Point GridWorld::get_centroid_in_polygon(Polygon _p) {
...
void GridWorld::output_centroid_in_polygon(Polygon _p) {
#ifdef GRIDWORLD_DEBUG
    printf("<");
    for (int x_ind = 0; x_ind < x_num; x_ind++){
        printf("-");
